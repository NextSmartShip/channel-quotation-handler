...
export function handleYuntuExcelJson(json) {
  ...
  json.forEach((row) => {
    ...
    const startWeight =
      weights.length > 0 ? parseFloat(weights[0], 10) * 1000 : "";
    const endWeight =
      weights.length > 1 ? parseFloat(weights[1], 10) * 1000 : "";

    items.push({
      mode:
        firstWeight > 0 ? COST_MODE.ContinuedUnitPrice : COST_MODE.UnitPrice,
      country_code: code,
